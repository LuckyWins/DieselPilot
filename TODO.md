...
| Phone-side automation: a Shortcuts or Tasker rule sending bot commands | A phone geofence or a calendar event can send `/in 90` to the bot with no firmware involved at all, which is where that logic belongs — the phone already knows where it is and has spent years optimising the battery cost of knowing. Set aside as surplus: `/at` and `/in` already cover a preheat decided on the way out |
| Acting on a location sent to the bot | `AsyncTelegram2` already parses a location message into `TBMessage.location`, so a one-off "setting off now" point would be a few lines. Live location would not work without patching the library: Telegram delivers those as `edited_message`, and the parser handles only `message` and `callback_query`. Either way it adds little over typing `/in 90` |
| Conditional start, skipped when the garage is already warm | Real fuel saving in mild weather, and both the reading and the scheduler are already there. Surplus for a site heated as a one-off preheat rather than continuously |
| A message when the garage reaches a target temperature | Offered as the way round the rejected arrival-time preheat: report the measurement rather than model it. Still surplus to the progress reports, which already say whether it is warming up at all |
| A closing summary when a burn ends | Duration, fuel and temperature rise in one message. The progress reports already carry all three while it burns |
| A delayed retry after a failed ignition | A cold heater often lights on a later attempt, once the plug has cooled and the chamber cleared. Dropped as surplus, and it carries its own risk: every failed attempt leaves unburnt fuel in the chamber, which is the thing the rest of this firmware works to avoid |
